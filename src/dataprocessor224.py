...
#Verarbeitet die Bilder in 224-Pixel große bilder und teilt diese in Background und Rubbish ein
class image_transformer(object):
    ...
    #reduziere Koordinaten z1 und z3 um x1 und z2 und z4 um y1
    ...
    def process_image(self, datapoint, im, img_id, ann_id, list_dataclass, x1, y1, x2, y2, s, mode):
    ...
    #inkrementiere die X-Koordinaten um 224
    def step_x(self, x1, x2):
        return x1+224, x2+224

    #inkrementiere die Y-Koordinaten um 224
    def step_y(self, y1, y2):
        return y1+224, y2+224

    #Schneide je 224-pixel große Bilder aus, sodass alles erhalten bleibt
    def process_image_mode(self, datapoints, img_id, ann_id, data_path_original, data_path_new, s, mode):
        ...
        for i in range(0, len(datapoints)):
            ...
            while ((x1+224) <= width):
                if ((y1+224) <= height):
                    img_id, ann_id, list_dataclass = self.process_image(datapoint, im, img_id, ann_id, list_dataclass, x1, y1, x2, y2, s, mode)
